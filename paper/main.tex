...
The observed calcitonin--CEA correlation (r = 0.24, n = 34) represents a weak but non-trivial association. Concerns about imputation reliability are valid given this modest correlation. However, empirical evaluation demonstrates meaningful predictive utility:

\textbf{Evidence:} Including imputed CEA as a feature improved LightGBM accuracy by approximately 0.5 percentage points (from 96.73\% to 97.20\%) on the expanded dataset. While modest, this improvement occurs without any degradation in recall (96.1\% in both configurations), confirming that imputed CEA contributes discriminative information through interaction terms and ensemble aggregation without introducing noise.

\textbf{Safeguards:} The pipeline includes a binary \texttt{cea\_imputed\_flag} feature that explicitly preserves missingness information, enabling models to weight imputed versus observed values appropriately. The ``Without CEA'' ablation configuration confirms that model performance remains strong even without CEA features, demonstrating robustness.

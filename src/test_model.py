...
import shap
from explainability import generate_lime_explanation, create_lime_visualizations, compare_lime_shap, generate_counterfactuals
...
def run_lime_explainability(model, X_test_scaled, y_test, test_patients, model_type='logistic', dataset_type='expanded'):
    ...
    # SHAP comparison for the same samples
    shap_comparisons = []
    shap_values_by_idx = {}
    if shap is None:
        ...
        if explainer is not None:
            for exp_data in lime_explanations:
                ...
                try:
                    ...
                    comparison = compare_lime_shap(exp_data, shap_vector, lime_charts_dir)
                    if comparison is not None:
                        shap_comparisons.append(comparison)
                except Exception as e:
                    print(f"Failed to compute SHAP for sample {sample_idx}: {e}")

    # global LIME vs SHAP comparison chart
    ...
    # counterfactual analysis for high-risk predictions
...
if __name__ == "__main__":
    ...
    parser.add_argument('--explain', action='store_true',
                       help='generate full explainability report (LIME + SHAP comparison + counterfactuals)')

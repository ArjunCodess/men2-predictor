...
def print_test_metrics(model, X_test, y_test, model_type='logistic', dataset_type='expanded'):
    ...
    with open(results_file, 'w') as f:
        ...
        f.write(f"Average Precision: {metrics['average_precision']:.4f} (95% CI: [{ci_results['avg_precision']['ci_lower']:.4f}, {ci_results['avg_precision']['ci_upper']:.4f}])\n")
        f.write("=" * 60 + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-explain', action='store_true',
                       help='skip SHAP + LIME explainability outputs')
    ...
    if not args.no_explain:
        run_explainability(model, X_test_scaled, y_test, test_patients, model_type, dataset_type)
